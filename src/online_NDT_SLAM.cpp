...
This NDT algorithm is a work in progress, it is currently significantly slower than ICP for some unknown reason.
...
void DownsampleAndFilter(pcl::PointCloud<pcl::PointXYZI>::Ptr& input) {
	pcl::PointCloud<pcl::PointXYZI>::Ptr input_proc (new pcl::PointCloud<pcl::PointXYZI>);
	...
	sor1.setLeafSize(0.2f, 0.2f, 0.2f); // TODO: Other filtering methods? // 0.05 works well
...
{	
	if (count == 0) { // The first time you loop through, you need to save cloud_0
		...
		// (1) RUN NDT ON CLOUD_0 and CLOUD_1
		ndt.setInputTarget(cloud_0);
		ndt.setInputSource(cloud_1);
		...
		std::cout << "Trans Epsilon: " << ndt.getFitnessScore() << std::endl;

		std::cout << "Transforming Clouds... " << std::endl;
		iter_transform = ndt.getFinalTransformation();
		map_transform = map_transform * iter_transform; // Transforming all clouds into cloud_0 frame

		pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_1_trans (new pcl::PointCloud<pcl::PointXYZI>);
		...
		*cloud_0 = *cloud_1; // set cloud_0 (previous) = cloud_1 (current)
		...
		// Publish World-->ndt_odom Transform
		...
		*/ 
		...
		br->sendTransform(tf::StampedTransform(transform, ros::Time::now(), "world", "ndt_odom"));
...
int main(int argc, char** argv)
{
	...
	//// SET PARAMETERS FOR NDT | This can be tricky to get right
		// NDT has several termination criteria (ICP will finish if any of the three are satisfied)
